...
let rec sexpr_to_string (ty, sx) = 
  ...
  | SListLit (ty, pl)  -> "SListLit: \n Type: " ^ typ_to_string ty ^ 
                          "\n List: " ^ plist_to_string pl
  | STupleLit (tyl, pl) ->  "STupleLit: \n Types: " ^ tylist_to_string tyl ^ 
                            "\n List: " ^ plist_to_string pl
  | STableLit (tyl, pll) -> 
  ...
  | SBinop (sx1, binop, sx2) -> sexpr_to_string sx1 ^ biop_to_string binop ^ sexpr_to_string sx2
  | SUnop (uop, sx) -> unop_to_string uop ^ sexpr_to_string sx
  | SIfExpr (sx1, sx2, sx3) -> "if( " ^ sexpr_to_string sx1 ^ "\nthen { " ^
                                sexpr_to_string sx2 ^ "}\n else {" ^
                                sexpr_to_string sx3 ^ "}\n"
  | SLambda (bl, sx) ->  "(" ^ blist_to_string bl ^ ") => " ^ "{ " ^ sexpr_to_string sx ^ " }"
  | SVar (s) -> "Variable " ^ s
  | SApply (sx, s, sxl) -> sexpr_to_string sx ^ "." ^ s ^ "(" ^ sexprlist_to_string sxl ^ ")"
  | SCall (s, sxl) -> s ^ "(" ^ sexprlist_to_string sxl  ^ ")"
  | SNoExp -> "SNoExpr"
  in "Sexpr( " ^ typ_to_string ty ^ ", " ^ sx_to_string sx ^ " )"



let rec sslist_to_string sl = 
  let rec sstmt_to_string = function
    SWhile(sx, stl) -> "while( " ^ sexpr_to_string sx ^ ")\n{" ^ sslist_to_string stl ^ "}\n"
  | SIf(sx, stl1, stl2) -> "if( " ^ sexpr_to_string sx ^ "\nthen { " ^
                            sslist_to_string stl1 ^ "}\n else {" ^
                            sslist_to_string stl2 ^ "}\n"
  | SReturn (sx) -> "Sreturn " ^ sexpr_to_string sx
  | SBreak -> "SBreak"
  | SDeclare(ty, str, sx) -> "Sdeclare: " ^ typ_to_string ty ^ " " ^ str ^ 
                              " = " ^ sexpr_to_string sx 
