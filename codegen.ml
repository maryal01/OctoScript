    ...
    *)
    ...
    let rec expr builder env ((etype, e) : sexpr) =
      ...
      | SCall ("add", args) ->
          ...
          let list_type =
            L.build_load
              (L.build_struct_gep listt 0 "tmp" builder)
              "tmp" builder
          in
          let length =
            L.build_load
              (L.build_struct_gep listt 1 "tmp" builder)
              "tmp" builder
          in
          let elem_type =
            L.build_load
              (L.build_struct_gep listt 2 "tmp" builder)
              "tmp" builder
          in
          let old_list = L.build_struct_gep listt 3 "tmp" builder in
          let len =
